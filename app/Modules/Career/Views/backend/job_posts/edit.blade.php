...
<form action="{{ route('jobposts.update', $jobPost->id) }}" method="POST">
  @csrf
  @method('PUT')
  <div class="mb-3 row">
    <label for="title" class="col-sm-2 col-form-label">Title</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="title" name="title" maxlength="100" value="{{$jobPost->title}}">
      @error('title')
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <textarea type="text" class="form-control" id="myeditor" maxlength="5000" name="description">{{$jobPost->description}}</textarea>
      @error('description')
      <div class="text-danger">{{ $message }}</div>
      @enderror
    </div>
    ...
    <div class="col-sm-10">
      <select class="form-control" id="department"  name="department">
        ...
        <option value="{{$department->id}}"
          @if($jobPost->dept_id == $department->id )
          selected
          @endif
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="skills" name="skills" maxlength="250" value="{{$jobPost->skills}}">
      @error('skills')
      <div class="text-danger">{{ $message }}</div>
      @enderror
    </div>
  ...
  <!-- <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <input type="date" class="form-control" id="posted_date" name="posted_date" value="{{$jobPost->posted_date}}">
    </div>
  </div> -->
  <div class="mb-3 row">
    <label for="expiry_date" class="col-sm-2 col-form-label">Expiry Date</label>
    <div class="col-sm-10">
      <input type="date" class="form-control" id="expiry_date" name="expiry_date" value="{{$jobPost->expiry_date}}">
      @error('expiry_date')
      <div class="text-danger">{{ $message }}</div>
      @enderror
    </div>
  ...
  <div class="mb-3 row">
    ...
    <div class="col-sm-10">
      <select class="form-control" name="status">
        <option value="1" @if($jobPost->is_active == 1 )
          selected
          @endif>Active </option>
        <option value="0" @if($jobPost->is_active == 0 )
          selected
